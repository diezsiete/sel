        ...
        if($message->getChildClass() === Referencia::class) {
           if($hv = $this->hvRepo->find($message->getHvId())) {
               $this->handleRequest($this->em, $message, function () use ($hv) {
                   ...
                   return $this->client->saveReferencias($hv);
               });
           }
        } else {
            $childNormalized = $message->getChildNormalized();
            /** @var HvEntity $hvEntity */
            $hvEntity = $this->denormalizer->denormalize(
                $childNormalized, $message->getChildClass(), null, ['groups' => ['messenger:hv-child:put', 'napi:hv-child:put']]
            );
            $this->handleRequest($this->em, $message, function () use($hvEntity) {
                try {
                    return $this->client->putChild($hvEntity);
