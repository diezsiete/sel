...
class UpdateHvChildsInNovasoftHandler implements MessageHandlerInterface
{
    ...
    public function __invoke(UpdateHvChildsInNovasoft $message)
    {
        try {
            $this->em->clear();
            $solicitud = $this->getSolicitud($this->em, $message->getSolicitudId(), $this->logger);

            $data = $this->normalizeHvChilds($solicitud->getHv(), $message->getChildClass(), $this->normalizer);

            $this->subscribeToStreamResponse($this->eventDispatcher);

            $this->logger->debug("updating childs of class '{$message->getChildClass()}'", ['data' => $data]);

            $response = $this->scraper->updateChilds($data);
